    ...
    App::App() {
        ...
        auto renderPipeline =
            RenderPipelineBuilder::New()
                .setVertexStateReq(VertexState(shader).addVertexLayout(Vertex::GetLayout()))
                .setFragmentStateReq(FragmentState(shader).addColorTargetState(ColorTargetState(m_Context->getSurfaceFormat())))
                .setDepthStencilState(DepthStencilState())
                .build(pipelineLayout, m_Context);
        auto renderBundleEncoder =
            RenderBundleEncoderBuilder::New()
                .addColorFormatReq(m_Context->getSurfaceFormat())
                .setDepthStencilFormat(m_ContextDepthBuffer->getFormat())
                .build(m_Context);
    ...
    void App::renderFrame(const std::shared_ptr<Framebuffer> &frame) {
        ...
        std::shared_ptr<RenderPass> renderPass = RenderPassBuilder::New()
                                                     ...
                                                     .addColorAttachment(ColorAttachment::New(m_Framebuffer)
                                                                             .setClearColor(0.1f, 0.3f, 0.3f, 1.0f))
                                                     .setDepthStencilAttachment(DepthStencilAttachment::New(m_Depthbuffer))
                                                     .build(encoder);
        renderPass->executeRenderBundles({m_RenderBundle});
        renderPass->finish();
