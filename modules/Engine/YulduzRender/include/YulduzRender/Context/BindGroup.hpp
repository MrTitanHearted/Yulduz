...
namespace Yulduz {
    ...
    class BindGroupLayoutBuilder {
       public:
        ...
        inline BindGroupLayoutBuilder &addStorageTexture2D(std::uint32_t binding) {
            return addStorageTexture2D(binding, StorageTextureEntry(ShaderStage::Fragment | ShaderStage::Compute, TextureFormat::RGBA8Unorm));
        }
        inline BindGroupLayoutBuilder &addStorageTexture3D(std::uint32_t binding) {
    ...
    class BindGroupBuilder {
       public:
        ...
        BindGroupBuilder &addStorageBuffer(std::uint32_t binding, const std::shared_ptr<StorageBuffer> &buffer);
        BindGroupBuilder &addUniformBuffer(std::uint32_t binding, const std::shared_ptr<UniformBuffer> &buffer);
        BindGroupBuilder &addSampler(std::uint32_t binding, const std::shared_ptr<Sampler> &sampler);
        BindGroupBuilder &addTexture(std::uint32_t binding, const std::shared_ptr<Texture> &texture);
