...
namespace Yulduz {
    ...
    ColorAttachment &ColorAttachment::setTarget(const std::shared_ptr<Texture> &target) {
        m_Attachment.view = target->getView();
        return *this;
    }

    ColorAttachment &ColorAttachment::setResolveTarget(const std::shared_ptr<Texture> &resolveTarget) {
    ...
    ColorAttachment &ColorAttachment::setClearColor(float r, float g, float b, float a) {
    ...
    DepthStencilAttachment &DepthStencilAttachment::setTarget(const std::shared_ptr<Texture> &target) {
        m_Attachment.view = target->getView();
        return *this;
    }
    ...
    void RenderPass::setVertexBuffer(std::uint32_t slot, const std::shared_ptr<VertexBuffer> &buffer) {
    ...
    void RenderPass::setBindGroup(std::uint32_t groupIndex, const std::shared_ptr<BindGroup> &bindGroup) {
        wgpuRenderPassEncoderSetBindGroup(m_RenderPassEncoder, groupIndex, bindGroup->get(), 0, nullptr);
    }

    void RenderPass::draw(std::uint32_t slot, const std::shared_ptr<VertexBuffer> &buffer) {
        wgpuRenderPassEncoderSetVertexBuffer(m_RenderPassEncoder, slot, buffer->get(), 0, buffer->getSize());
