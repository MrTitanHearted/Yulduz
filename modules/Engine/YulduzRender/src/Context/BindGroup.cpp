...
namespace Yulduz {
    ...
    BindGroupBuilder &BindGroupBuilder::addStorageBuffer(std::uint32_t binding, const std::shared_ptr<StorageBuffer> &buffer) {
        m_Entries.emplace_back(WGPUBindGroupEntry{.binding = binding, .buffer = buffer->get(), .size = buffer->getSize()});
        m_Buffers.emplace_back(buffer);
        return *this;
    }

    BindGroupBuilder &BindGroupBuilder::addUniformBuffer(std::uint32_t binding, const std::shared_ptr<UniformBuffer> &buffer) {
        m_Entries.emplace_back(WGPUBindGroupEntry{.binding = binding, .buffer = buffer->get(), .size = buffer->getSize()});
        m_Buffers.emplace_back(buffer);
        return *this;
    }

    BindGroupBuilder &BindGroupBuilder::addSampler(std::uint32_t binding, const std::shared_ptr<Sampler> &sampler) {
        m_Entries.emplace_back(WGPUBindGroupEntry{.binding = binding, .sampler = sampler->get()});
        m_Samplers.emplace_back(sampler);
        return *this;
    }

    BindGroupBuilder &BindGroupBuilder::addTexture(std::uint32_t binding, const std::shared_ptr<Texture> &texture) {
